{
  calendar-notify = {
    ...
    mkText = p: /*haskell*/ ''
      ...
      loadFromBins [ "${p.khal}", "${p.libnotify}", "${p.coreutils}", "${p.mpv}" ]
  ...
  calendar-status = {
    ...
    libraries = [ "bytestring" "chronos" "mtl" "shh" "text" "torsor" ];
    mkText = p: /*haskell*/ ''
      module Main where
      ...
      loadFromBins ["${p.coreutils}", "${p.khal}"]
  ...
  deploy = {
    ...
    mkText = p: /*haskell*/ ''
      ...
      loadFromBins ["${p.coreutils}", "${p.git}", "${p.hostname}", "${p.nvd}", "${p.openssh}"]
